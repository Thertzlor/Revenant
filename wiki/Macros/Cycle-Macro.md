...
### Complete Syntax:
>`{ type="cycle"|"c",  <steps...>,  [, cn/cancel=<number>, i/interval=<number>, limit=<number>, finish=<option|macro>, inherit=<option>, range=<option>] }`
```lua
...
k.m3 = { type="cycle", {"macro_a"}, {"macro_b"}  }
...
```
# Options
Besides the [General Macro Options](./Macro-Overview#general-macro-options) the Cycle Macro offers the following options to customize behavior:
## cancel
* shorthand: `cn`
...
* **`0`** *(default)* = The cycle will not cancel unless it is externally cancelled from a [Control Macro](./Control-Macro) or the entire profile is reloaded.  
...
k.m3 = { type="cycle", "a","b","c", cancel=1 }

-- resets back to "a" after one second of inactivity.
k.m3 = { type="cycle", "a","b","c", cancel=1000 }

-- resets back to "a" whenever another button is pressed OR after one second of inactivity.
k.m3 = { type="cycle", "a","b","c", cancel=-1000 }

```
The Configuration setting [separateDeviceCycles](./Options-Documentation#separatedevicecycles) controls if button presses on one device family can cancel cycles on another device. 
...
k.m3 = { type="cycle", "a","b","c", range = {0,2} }
...
k.m3 = { type="cycle", "a","b","c", range = {0,0,2} }
...
k.m3 = { type="cycle", "a","b","c", limit=3, finish="stall" }

-- After reaching "c" for the third time, pressing this button does nothing.
k.m4 = { type="cycle", "a","b","c", limit=3, finish="end" }
...
k.m5 = { type="cycle", "a","b","c", limit=3, finish="reset" }
...
k.m3 = { type="cycle",  "a","b",{ type="cycle", "c","d","e", inherit="status"}, cancel=2000}
...
k.m4 = { type="cycle",  "f","g",{ type="cycle", "h","i","j", inherit="none"}, cancel=2000}
