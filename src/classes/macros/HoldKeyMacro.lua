...
function HoldKeyMacro:parseInstructions()
    ...
    local function finalIteration()
        ...
        else command[#command + 1] = lastCommand end
        ...
        for i = 1, #command do local cmd = command[i]
            if type(cmd) == "number" then
                ...
                if #workTab ~= 0 then --handling the different types of delay definitions
                    if stagMode == "absolute" then currentDelay = defaultDelay
                    else
        ...
        for i = 1, #self.command do local finalCommand = self.command[i][2]
            if type(finalCommand) == "table" and finalCommand._ref then local ref = finalCommand._ref
                self.command[i] = { ref }
    ...
    for i = 1, #originalCommands do local cmd = originalCommands[i]
        ...
        elseif commandType == "table" then --any other table has to be a macro
            ...
            if tableType == "group" then macroClass = rv:classImport('GroupMacro')
            elseif tableType == "macro" then macroClass = rv.tbl:getMacroClass(cmd) end
