...
function TableUtilitiesModule:hasProperties(tb)
    for i, _ in pairs(tb) do
        if type(i) == "string" and not self:find(tl.stringPresets.internalProps, i) then
            return true
        end
    end
    return false
...
function TableUtilitiesModule:sameContent(t1, t2)
    ...
    for k, v in pairs(t1) do
        t1_num = t1_num + 1
        if not t2[k] or type(t2[k]) ~= type(t1[k]) then
            return false
        end
        if t2[k] and not self:find(tl.stringPresets.internalPropsName, k) then
            if type(v) == "table" and not self:sameContent(t1[k], t2[k]) then
                return false
            end
        end
    end
...
function TableUtilitiesModule:indexTables(macroTarget, tar, scope, key, parent, typeCast)
    ...
    for k, n in pairs(tar) do
        if type(n) == "table" then
            if self:find({"start", "key", "exit"}, key) then
                parent = k
            end
...
function TableUtilitiesModule:cycleIndex(dex, num, current)
    ...
    elseif type(num) ~= "number" then
        if type(num) ~= "string" or not current then
            return 1
        end
        local sign = sub(num, 1, 1)
        local parsedNum = tonumber(sub(num, 2))
        if not parsedNum or (sign ~= "+" and sign ~= "-") then
            return current
        end
