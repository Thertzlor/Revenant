...
local function _testMode(stat, modi, lMod, fam, manual)
    ...
    elseif type(testMode) == "table" then ---checking multiple modes
        ...
        for i = 1, #testMode do local obj = testMode[i] ---separating mode cheks into positive and negative checks
            local target = positives
            if type(obj) == "number" and obj < 0 then target = negatives
            elseif type(obj) == "string" and sub(obj, 1, 1) == "-" then target = negatives end
            target[#target + 1] = obj
        end
...
local function _testKey(stat, modifierString, modifiers)
    ...
    if (modifierString == "no" and (modifiers == nil or modifiers == 0 or not next(modifiers))) or
        (modifierString ~= "no" and (modifierString == nil or modifierString == 0 or modifierString == "")) then
    ...
    elseif type(modifiers) == "table" and type(modifierString) == "string" then
        ...
        if not strict and modifiers[modifierString] == true then testPass = true else
            testPass = true
            local matchedCombinations = {} ---@type table<string,true>

            for mod in gmatch(modifierString, "%a%a") do
                if modifiers[mod] ~= true then testPass = false break end --false if q requirement isn't met
            ...
            if strict then --making sure no additional modifiers are pressed, ignoring "lock" type modifiers
                for modKey in pairs(modifiers) do
                    if sub(modKey, 1, 1) ~= "g" and sub(modKey, 2, 2) ~= "l" and matchedCombinations[modKey] ~= true then testPass = false break end
                end
            end
...
local function _testFlags(flagName, negate)
    ...
    elseif rv.scriptStates.flags[flagName] then return tres end
    return not tres
end
...
local function logicGate(truthTable, mode, eval)
    ...
    for i = 1, #truthTable do local obj = truthTable[i] --iterating through all results
        ...
        elseif obj == true then passes[#passes + 1] = 1 end
...
local function _conditionEvaluation(t_cond, key, virtu, fam, t_ident)
    ...
    local function _recursiveTest(testInput) --evaluating the "test" conditions of a key.(recursive)
        ...
        elseif type(testDefinition) == "number" then
            if testDefinition > 0 then testDefinition = fam .. testDefinition
            else testDefinition = "-" .. fam .. abs(testDefinition) end
        end
        ...
        local function testPreviouslyPressed(keyName, negate)
            ...
            for g = 1, #testRay do
                ...
                if (nopster == false and _singleTest(unit, rv.keyStates.lastKeysDown[#rv.keyStates.lastKeysDown - g + virtualOffset], fam))
                    or (nopster == true and (not _singleTest(unit, rv.keyStates.lastKeysDown[#rv.keyStates.lastKeysDown - g + virtualOffset], fam))) then
        ...
        if type(testDefinition) == "string" then
            ...
            if prefix == "-" then return testCurrentlyPressed(sub(testDefinition, 2), true)
            elseif prefix == "^" then return testPreviouslyPressed(sub(testDefinition, 2))
            elseif prefix == "|" then return testPreviouslyPressed(sub(testDefinition, 2), true)
            elseif prefix == ":" then return _testSequence(sub(testDefinition, 2))
            elseif prefix == "~" then return _testSequence(sub(testDefinition, 2), true)
            elseif prefix == "." then return _testFlags(sub(testDefinition, 2))
            elseif prefix == "*" then return _testFlags(sub(testDefinition, 2), true)
            else return testCurrentlyPressed(testDefinition) end --Just executing the normal test
        end
        return false
    end
    ...
    if (rv.scriptStates.currentButton == keyNum or virtualState) and (virtualState or state[fam].blockedKey ~= keyNum) then
        ...
        if not virtualState then
            if buttonDirection == "down" then
                buttonCheck = _testShift(meta, options.gshift or config.defaultShift, lastShift) and
                    _testMode(meta, options.mode or config.defaultMode, lastMode, fam) and
            ...
            buttonCheck = ((not options.gshift) or _testShift(meta, options.gshift or config.defaultShift, lastShift)) and
                ((not options.mode) or _testMode(meta, options.mode or config.defaultMode, lastMode, fam)) and
        ...
        if buttonCheck then
            if buttonDirection == "down" then meta.allPassed = true
            elseif buttonDirection == "up" then meta.allPassed = nil end
