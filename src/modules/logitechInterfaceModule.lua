...
function LogitechInterfaceModule:_modeSelect(targ, fam)
  if type(fam) == "string" and fam == "all" then
    ...
    for g = 1, #famArr do
      self:_modeSelect(targ, famArr[g])
    end
  elseif type(fam) == "table" then
    for g = 1, #fam do
      self:_modeSelect(targ, fam[g])
    end
  else
    ...
    elseif targ <= tl.deviceState[fam].modeCount then --else cycle until you reach the target mode
      ...
      self:_modeSelect(tl.deviceState[fam].modeCount, fam)
    end
    if not tl.config.keepNameOnLCD then
      tl:put(
        "changed to mode '" ..
          (tl.deviceState[fam].modeConfig[tl.deviceState[fam].modus][1] or tl.deviceState[fam].modus) .. "' for " .. self.unToken[fam]
      )
    else
      self:putNoLCD(
        "changed to mode '" ..
...
function LogitechInterfaceModule:_toggleMode(md, fam) --
  if type(fam) == "string" and fam == "all" then
    local famArr = {"m", "a", "l", "k"}
    for g = 1, #famArr do
      self:_toggleMode(md, famArr[g])
    end
  elseif type(fam) == "table" then
    for g = 1, #fam do
      self:_toggleMode(md, fam[g])
    end
  else
...
function LogitechInterfaceModule:_temporaryMode(md, num, fam)
  if fam == "all" then
    local famArr = {"m", "a", "l", "k"}
    for g = 1, #famArr do
      self:_temporaryMode(md, num, famArr[g])
    end
  elseif type(fam) == "table" then
    for g = 1, #fam do
      self:_temporaryMode(md, num, fam[g])
    end
  else
...
function LogitechInterfaceModule:externalMacroWrapper(cmd, dir, dirMatch)
  if type(cmd) == "table" and cmd.play then
    if cmd.play == "toggle" then
      self:_toggleExternalMacro(cmd)
    elseif cmd.play == "hold" then
      self:_toggleExternalMacro(cmd, dir)
    end
  elseif dirMatch then
    self:_playExternalMacro(cmd)
  end
end
