        ...
        if type(k) == "string" and not (tl.config.rename[k] or tl.keyStates.unRename[k]) then
                        ...
                        return false, "'" ..
                            v ..
                                "' is not a valid value for " ..
                                    propTerm ..
                                        " '" .. k .. "'. Accepted values are: '" .. concat(def.values, "' ,'") .. "'"
                    end
            ...
            if type(v) == "string" then
                ...
                if illegalStart then
                    return false, "Found string value starting with illegal character '" ..
                        illegalStart .. "' on " .. propTerm .. " " .. k
                end
            end
            if
                def.range and type(v) == "number" and
                    ((def.range[1] and v < def.range[1]) or (def.range[2] and v > def.range[2]))
             then
                ...
                for i, c in pairs(v) do
                    if not tl.tbl:find(tl.stringPresets.internalPropsName, i) then
                        if def.tableKeys and not tl.tbl:find(def.tableKeys, type(i)) then
                            return false, "Table on " ..
                                propTerm .. " '" .. k .. "' contains key of invalid type " .. type(i)
                        end
                        if def.tableTypes and not tl.tbl:find(def.tableTypes, type(c)) then
                            return false, "Table on " ..
                                propTerm .. " '" .. k .. "' contains value of invalid type " .. type(i)
                        end
                        if def.tableVals and not tl.tbl:find(def.tableVals, c) then
                            return false, "'" ..
                                c ..
                                    "' is not a valid value for entries on" ..
                                        propTerm ..
                                            " '" ..
                                                k .. "'. Accepted values are: '" .. concat(def.tableVals, "' ,'") .. "'"
                        end
                    end
                end
            end
...
function LintingModule:KeyLinter(table, parentKey, typeCast)
    ...
    if res == false then
        self.lintErrors[tl.keyStates.unRename[parentKey] or tostring(parentKey)] =
            "LINT ERROR: " .. mes .. " on '" .. (tl.config.rename[parentKey] or tostring(parentKey)) .. "'"
    end
    return res
end
...
function LintingModule:configLinter(table, profileName)
    ...
    if res == false then
        self.configLintErrors[profileName] =
            "CONFIGURATION ERROR: " .. mes .. " on configuration for '" .. profileName .. "'"
    end
    return res
end
...
LintingModule.optionsDefinitions = {
    ...
    pollFamily = {
        type = "string",
        values = {tl.config.hubMode and "kb" or "lhc", "kb", "mouse"}
    },
    randomActionDeviation = {
        type = "number",
        range = {0}
    },
    randomKeyDeviation = {
    ...
    modeSort = {
        type = {"string", "table"},
        values = {"reverse", "standard"},
        tableKeys = "number",
        tableTypes = {"string", "number"}
    },
    shiftSort = {
        type = {"string", "table"},
        values = {"reverse", "standard"},
    ...
    handleKeyConflicts = {
        type = {"string", "number"},
        values = {"prepend", "append", "discard", "overwrite", "overwriteAll", "discardAll"},
        range = {0}
    },
    handleOptionConflicts = {
        type = {"string", "number"},
        values = {"replaceDuplicates", "useFirst", "useLast", "discardDuplicates"},
        range = {0}
    },
    handleDocumentationConflicts = {
        type = {"string", "number"},
        values = {"replaceDuplicates", "useFirst", "useLast", "discardDuplicates"},
        range = {0}
    },
    handleLibraryConflicts = {
        type = {"string", "number"},
        values = {"replaceDuplicates", "useFirst", "useLast", "discardDuplicates"},
        range = {0}
    },
