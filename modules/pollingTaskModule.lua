...
function tl.polling:poll(event, arg, st)
  ...
  if event == "M_PRESSED" and arg ~= self.pollControls.activeState then
    if self.pollControls.stateTimer ~= nil and t >= self.pollControls.stateTimer then self.pollControls.stateTimer = nil end
  ...
  elseif event == "M_RELEASED" and arg == self.pollControls.activeState then
    ...
    if self.pollControls.pollRateC == self.pollControls.pollRateCI then
      ...
      self.pollControls.pollRateSum=0;self.pollControls.pollRateC=0
    end
